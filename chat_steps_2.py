...
with open('chat_flow_2.yaml') as f:
...
class OfferReappraisal(ChatStep):
    ...
    def gpt_reappraise(self, value_prompt=None):
        '''sends issue and values to GPT and asks for a reappraisal'''
        # d = {"connection": "gpt_connection"}
        if not value_prompt:# or value_prompt not in d.keys():
            prompt_type = "gpt_general"
        else:
            # make value_prompt lowercase and replace spaces with underscores
            prompt_type = "gpt_" + value_prompt.lower().replace(" ", "_").replace("-", "_")
        print(f"Prompt type: {prompt_type}")
        gpt_content = chat_flow[self.name]["speaker"][prompt_type]
